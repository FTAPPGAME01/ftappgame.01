...
io.on('connection', (socket) => {
    ...
    socket.on('updateState', (updatedState) => {
        ...
        // Escuchar el evento de recarga de puntos
        socket.on('reloadPoints', (data) => {
            const { player, amount } = data;
    
            // Aquí podrías validar el PIN o realizar otras verificaciones antes de proceder
    
            // Emitir la actualización de puntos a todos los clientes
            io.emit('updatePlayerPoints', { player, newScore: amount });
        });
    });
